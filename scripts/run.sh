#!/bin/bash
# Run baseline (constant ceiling) vs treatment (exponential ceiling) comparison.
...
# --- Run function ---
# BLIS must run from its own directory to find model configs.
...
# Instance count per workload (matches campaign setup)
get_instances() {
  local wl_name=$1
  case $wl_name in
    *_5inst) echo 5 ;;
...
echo ""
echo "════════════════════════════════════════════════════"
echo "  PHASE 2: TREATMENT (exponential ceiling patch)"
echo "════════════════════════════════════════════════════"
...
git checkout --quiet -- .
git apply "$PATCH"
echo "Rebuilding BLIS with exponential ceiling..."
go build -o blis main.go
echo "Build complete."
